...
public class IslandService : IIslandService 
{
	...
	private void CreateRivers()
	{
		...
		{
			Corner q = WorldGen.AppMap.Corners.Values.ElementAt(Random.Range(0, WorldGen.AppMap.Corners.Values.Count - 1));
	...
	private void RedistributeElevation()
	{
		...
		var locations = WorldGen.AppMap.Corners.Values.Where (x => !x.Ocean).OrderBy (x => x.Elevation).ToArray ();

		for (int i = 0; i < locations.Count(); i++)
		{
			double y = (double)i / (locations.Count() - 1);

			var x = 1.04880885 - Mathf.Sqrt((float)scaleFactor * (float)(1f - y));
			if (x > 1.0)
				x = 1.0;
	...
	private bool IsLandShape(Vector2 point)
	{
		...
		double startAngle = Random.value * 2 * Mathf.PI;
		double dipAngle = Random.value * 2 * Mathf.PI;
		double dipWidth = Random.Range (2, 7) / 10;
